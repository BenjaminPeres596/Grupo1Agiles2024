...
export default function HomeScreen() {
    ...
    const foodPoints: FoodPoint[] = [
        ...
        { id: 4, name: "Atenas Parrilla Resto", latitude: -34.92476621004759, longitude: -57.949965283708394 },
        { id: 5, name: "Ese es mi pollo", latitude: -34.90639681023369, longitude: -57.924194197557235 },
    ];

    // Función para calcular la distancia entre dos puntos usando la fórmula de Haversine
    // Recibe latitud y longitud de dos puntos (lat1, lon1 y lat2, lon2)
    ...
    useEffect(() => {
        // Función autoejecutable para obtener la ubicación del usuario
        (async () => {
            ...
            try {
                // Obtener la ubicación actual con la mayor precisión
                ...
                // Filtrar restaurantes cercanos dentro de un radio de 1 km utilizando la fórmula de Haversine
                ...
                // Actualizar la lista de restaurantes cercanos y la ubicación del usuario
    ...
    return (
        <ParallaxScrollView
            ...
            <ThemedView style={styles.titleContainer}>
                <ThemedText type="title">Comer seguro, sin gluten</ThemedText> {/* Título de la pantalla */}
            </ThemedView>

            {/* Mapa */}
            <View style={styles.mapContainer}>
                {loading ? (
                    <ActivityIndicator size="large" color="#0000ff" /> // Indicador de carga mientras se obtiene la ubicación
                ) : location ? (
                    <MapView
                        style={styles.map}
                    ...
                    <ThemedText>No se pudo obtener la ubicación</ThemedText> // Mensaje si no se obtiene la ubicación
                )}
            </View>

            {/* Lista de restaurantes cercanos */}
            <ThemedView style={styles.restaurantsContainer}>
                <ThemedText type="title">Restaurantes cercanos:</ThemedText> {/* Título de la lista */}
                {nearbyRestaurants.length > 0 ? (
                    ...
                    <ThemedText>No se encontraron restaurantes cercanos.</ThemedText> // Mensaje si no hay restaurantes cercanos
                )}
            </ThemedView>
        </ParallaxScrollView>
    );
